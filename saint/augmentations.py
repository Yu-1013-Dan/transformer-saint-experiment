...
def embed_data_mask(x_categ, x_cont, cat_mask, con_mask,model,vision_dset=False):
    ...
    if model.categorical_embedding_type == 'saint':
        x_categ_with_offset = x_categ + model.categories_offset.type_as(x_categ)
        x_categ_enc = model.embeds(x_categ_with_offset)
    elif model.categorical_embedding_type == 'ft':
        x_categ_enc = model.categorical_embedder(x_categ)
    
    if model.numerical_embedding_type == 'mlp':
        n1,n2 = x_cont.shape
        x_cont_enc = torch.empty(n1,n2, model.dim, device=device)
        for i in range(model.num_continuous):
            x_cont_enc[:,i,:] = model.simple_MLP[i](x_cont[:,i].unsqueeze(-1))
    elif model.numerical_embedding_type == 'ple':
        x_cont_enc = model.numerical_embedder(x_cont)
    else:
    ...
    x_categ_enc = x_categ_enc.clone()
    x_cont_enc = x_cont_enc.clone()
